...
This concept, which is known as _overloading_, might seem somewhat alien to the APL programmer, who will be accustomed to defining functions that accept an arbitrary array. However, type checking, which is fundamental to .NET, requires that a method is called with the correct number of parameters, and that each parameter is of a predefined type. Overloading solves this issue.
